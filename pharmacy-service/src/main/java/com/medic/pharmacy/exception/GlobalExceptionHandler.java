package com.medic.pharmacy.exception;

import com.medic.pharmacy.dto.ApiError;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleNotFound(PrescriptionNotFoundException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body(ApiError.of(404, "Not Found", ex.getMessage()));
    }

    @ExceptionHandler(DrugInteractionException.class)
    public ResponseEntity<ApiError> handleInteraction(DrugInteractionException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
            .body(ApiError.of(409, "Drug Interaction Detected", ex.getMessage()));
    }
