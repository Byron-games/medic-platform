...
public class JwtService {
    ...
    /**
     * Generates a short-lived access token with user claims embedded. The API Gateway validates
    ...
    /**
     * Generates a long-lived refresh token (minimal claims — just subject + type). The actual
     * refresh token record is stored in the database.
     */
    ...
    private String buildToken(Map<String, Object> claims, long expiryMs) {
        ...
        return Jwts.builder().setClaims(claims).setIssuedAt(new Date(now))
                .setExpiration(new Date(now + expiryMs))
                .signWith(signingKey, SignatureAlgorithm.HS384).compact();
    }
