...
public class JwtGatewayFilter implements GlobalFilter, Ordered {
    ...
    private static final List<String> PUBLIC_PATHS = List.of("/api/v1/auth/login",
            "/api/v1/auth/register", "/api/v1/auth/refresh", "/api/v1/ussd", // USSD webhook —
                                                                             // Africa's Talking
                                                                             // sends POST here
            "/actuator/health", "/actuator/prometheus", "/v3/api-docs", "/swagger-ui");
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            claims = Jwts.parser().verifyWith(signingKey).build().parseSignedClaims(token)
                    .getPayload();
        ...
        ServerHttpRequest enrichedRequest =
                exchange.getRequest().mutate().header("X-User-Id", safeString(claims.get("userId")))
                        ...
                        .header("X-Facility-Name", safeString(claims.get("facilityName"))).build();
