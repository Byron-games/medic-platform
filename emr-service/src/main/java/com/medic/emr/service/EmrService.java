...
public class EmrService {
    ...
    public Page<RecordSummary> listForPatient(String mpiId, RecordType type,
                                               int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<MedicalRecord> records = type != null
            ? recordRepo.findByPatientMpiIdAndRecordTypeOrderByVisitDateDesc(mpiId, type, pageable)
    ...
    public MedicalRecordResponse create(CreateRecordRequest req) {
        ...
        String facilityName  = ctx.getFacilityName() != null ? ctx.getFacilityName() : "Unknown Facility";
        ...
        MedicalRecord record = MedicalRecord.builder()
            ...
            .plan(req.plan())
            .icd10Codes(req.icd10Codes())
            .networkShared(req.networkShared())
            .build();
        ...
        log.info("EMR record created: id={} patient={} type={} by={}",
            record.getId(), req.patientMpiId(), record.getRecordType(), clinicianName);
    ...
    public MedicalRecordResponse update(Long id, UpdateRecordRequest req) {
        ...
        if (!ctx.isAdmin() && !record.getClinicianId().equals(ctx.getUserId())) {
            throw new UnauthorisedException("Only the authoring clinician or an admin can edit this record");
        }

        if (req.recordType()    != null) record.setRecordType(req.recordType());
        if (req.chiefComplaint()!= null) record.setChiefComplaint(req.chiefComplaint());
        if (req.subjective()    != null) record.setSubjective(req.subjective());
        if (req.objective()     != null) record.setObjective(req.objective());
        if (req.assessment()    != null) record.setAssessment(req.assessment());
        if (req.plan()          != null) record.setPlan(req.plan());
        if (req.icd10Codes()    != null) record.setIcd10Codes(req.icd10Codes());
        if (req.networkShared() != null) record.setNetworkShared(req.networkShared());
    ...
    public List<VitalSignsResponse> getVitals(Long recordId) {
        ...
        return vitalsRepo.findByRecordIdOrderByRecordedAtDesc(recordId)
    ...
    public List<RecordSummary> getTimeline(String mpiId, int limit) {
        ...
        return recordRepo.findRecentByPatient(mpiId, pageable)
            .stream().map(RecordSummary::from).toList();
    }
    ...
    private VitalSigns buildVitals(VitalSignsRequest req, MedicalRecord record) {
        ...
        if (req.weightKg() != null && req.heightCm() != null
            ...
            BigDecimal heightM = req.heightCm().divide(BigDecimal.valueOf(100), 4, RoundingMode.HALF_UP);
            bmi = req.weightKg()
