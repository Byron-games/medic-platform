...
export default function PatientsPage() {
  ...
  const fetchPatients = useCallback(async (term: string, pg: number) => {
    ...
    try {
      ...
      setError(
        "Failed to load patients. Is the Patient Identity Service running?",
      );
    } finally {
      setLoading(false);
    }
            ...
            <table className="w-full text-sm min-w-[640px]">
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {patients.map((p) => (
                  ...
                  >
                    <td
                      className="px-4 py-3 font-mono text-xs text-[var(--accent)]
                      whitespace-nowrap"
                    >
                      {p.mpiId}
                    </td>
                    ...
                    <td
                      className="px-4 py-3 text-[var(--text-secondary)] font-mono text-xs
                      whitespace-nowrap"
                    >
                      {p.phoneNumber ?? "—"}
                    </td>
